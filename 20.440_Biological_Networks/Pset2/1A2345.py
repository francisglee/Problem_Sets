#!/usr/bin/env python
'''
Problem Set 1A2345: Model fitting and validation
-Write a function to randomly select 50% of the data for training, 25% for validation and 25% for assessment.
-Fit the polynomials with α = 1, 2,…,12 using only the training data. 
-For each α value, use your fit parameters from the training data fit and compute the loss function using the validation data. 
-For each value of α, plot the MSE values for the training set and validation set on the same plot. Describe the general relationship between the two sets of errors. What is the best value of α given the results observed here?  
'''
...
coefficient_dict_training, mse_dict_training, mse_dict_validation = {}, {}, {}

for a in range(1, len(alpha) + 1):
    coefficient_dict_training[a] = np.polyfit(training[0], training[1], a)
    mse_dict_training[a] = mse(training[0], training[1], coefficient_dict_training[a])
    mse_dict_validation[a] = mse(validation[0], validation[1], coefficient_dict_training[a])
        
mse_val = [] # y values for validation Loss function 
for item in alpha:
    mse_val.append(mse_dict_validation[item])
    
mse_tra = [] # y values for training Loss function
for item in alpha:
    mse_tra.append(mse_dict_training[item])

# plot fig
fig = plt.figure()
ax1 = fig.add_subplot(111)
ax1.scatter(alpha, mse_tra, c = 'b', marker = 'D', label = 'training')
ax1.scatter(alpha, mse_val, c = 'r', marker = '+', label = 'validation')

# format fig
plt.title('MSE of training vs. validation data')
plt.xlabel('alpha')
