#!/usr/bin/env python
'''
Problem Set 1A67: Model fitting and validation
-On a single plot, show the original data, the fitted curve that minimizes the training error, and the curve that minimizes the validation error.
-To determine the prediction error, compute the loss function using the optimal value of α on your assessment set. 
'''
...
for key, value in mse_dict_training.items(): # only works if there' one min...!!!
    if value == min(mse_dict_training.itervalues()): 
        alpha_min_tra = key

for key, value in mse_dict_validation.items(): # only works if there' one min...!!!
    if value == min(mse_dict_validation.itervalues()): 
        alpha_min_val = key 
...
# evaluate prediction error of lowest loss function of the assessment set.
coefficient_set = np.polyfit(assessment[0], assessment[0], alpha_min_val)
prediction_error = lossfunction(assessment[0], assessment[0], coefficient_set)

print "The prediction error of this function is", prediction_error
